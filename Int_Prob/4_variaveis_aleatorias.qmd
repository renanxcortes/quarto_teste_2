...
Um ponto interessante é que a definição de uma FDA poderia ser dada simplesmente por essas propriedades. Isto é, dada qualquer função $F(.)$ que respeite as propriedades acima, logo ela **será FDA de alguma variável aleatória existente** (mesmo sem mencioná-la!).
...
# Calculando as probabilidades das somas
somas <- 2:12
probabilidades <- sapply(somas, function(soma) {
  # Contar combinações possíveis que resultam na soma
  combinações <- expand.grid(dado1=1:6, dado2=1:6)
  total <- nrow(combinações)
  resultados_validos <- sum(rowSums(combinações) == soma)
