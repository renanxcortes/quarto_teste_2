...
Inverter a ordem! e_ii e a_ii

Seja $X_1, X_2, ..., X_p$ variáveis aleatórias e seja $\bs{X'} = [X_1, X_2, ..., X_p]$ toda a sua base de dados^[@johnson2007applied chama essa quantidade de "vetor aleatório". No entanto, imagino que pensar como sendo uma base de dados com "p" colunas mais intuitivo.] ter uma matriz de covariância $\bs{\Sigma}$ com autovalores $\lambda_1 \ge \lambda_2 \ge ... \ge \lambda_p \ge 0$ associada à esta base. Considere a combinação linear


$$
\begin{align*}
Y_1 = \bs{a_1'X} &= a_{11}X_1 + a_{12}X_2 + ... +  a_{1p}X_p \\
Y_2 = \bs{a_2'X} &= a_{21}X_1 + a_{22}X_2 + ... +  a_{2p}X_p \\
&\;\;\vdots \notag \\
Y_p = \bs{a_p'X} &= a_{p1}X_1 + a_{p2}X_2 + ... +  a_{pp}X_p
\end{align*}
$$

As quantidades $Y_i$'s são considerados os componentes principais


Observe que $Y_i$'s são novas variáveis criadas a partir das variáveis $X_i$'s originais. Além disso, os $Y_i$'s **não são correlacionados entre si**.
